...
import getProductName from '@salesforce/apex/NewLocalComplaintController.getProductName';
...
export default class NewLocalComplaint extends NavigationMixin(LightningElement) {
    ...
    @track allegationChosen = "";
    @track callCenterSolutionChosen = "";
    @track isOpenAllModal = false;
    @track isOpenCallCenterSolutionModal = false;
    @track isOpenModal = false;
    @track productNameValue;

    // GET DATA FROM APEX
    @wire (getProductName, { caseId: '$recordId' })
    prodNameValue(result){
        this.productNameValue = result.data;
        console.log('this.productNameValue ------- : ' + this.productNameValue);
    }

    //EXECUTED AFTER SAVE IS SUCCESSFUL
    //SHOWS SUCCESS MESSAGE AND REDIRECTS TO THE DETAIL PAGE OF THE CREATED RECORD
    handleLocalComplaintCreated(e){
    ...
    handleCloseAllegationModal(event) {
        this.isOpenAllModal = event.detail;
        this.isOpenModal = false;
    }

    //TO EXIT THE CUSTOM LOOKUP COMPONENT FOR THE CALL CENTER SOLUTION FIELD AND COME BACK TO THE PARENT
    handleCloseCallCenterSolutionModal(event) {
        this.isOpenCallCenterSolutionModal = event.detail;
        this.isOpenModal = false;
    }
    ...
    handleSubmit(event){
        ...
        console.log('fieldssss --------- : ' + JSON.stringify(fields));
